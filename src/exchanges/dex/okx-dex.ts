...
export class OKXDEXExchange {
  private client: AxiosInstance;
  private config: ExchangeConfig;
  private chainConfig: ChainConfig;
  
  // OKX DEX API base URL
  private readonly OKX_DEX_API_BASE = 'https://www.okx.com/api/v5';
  ...
  private async getQuote(fromToken: string, toToken: string, amount: string): Promise<any> {
    try {
      const response = await this.client.get('/dex/aggregator/quote', {
        params: {
      ...
      console.error('Error getting OKX DEX quote:', error);
      throw error;
    }
  }
  ...
  async getPrice(symbol: string, chainId?: number): Promise<ExchangeResult> {
    try {
      ...
      if (!quote || !quote.data || !quote.data[0]) {
        return {
          exchange: 'OKX DEX',
          success: false,
          error: 'Invalid quote received from OKX DEX'
        };
      }
      ...
      // Calculate price
      const price = Number(quoteData.toTokenAmount) / Number(quoteData.fromTokenAmount);
